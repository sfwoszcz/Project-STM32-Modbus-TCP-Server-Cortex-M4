...
- dedicated FC20, FC21, FC22, FC23, and FC43/14 shared-PDU, TCP, RTU, master, transaction, boundary, and malformed-frame tests
...
- `mb_process_pdu()` dispatches shared function codes, including FC20 Read File Record, FC21 Write File Record, FC22 Mask Write Register, FC23 Read/Write Multiple Registers, and FC43/14 Read Device Identification, and creates normal or exception response PDUs without TCP-specific framing.
...
- `mbrtum_build_*_request()` creates complete RTU master requests, including FC11, FC20, FC21, FC22, FC23, and FC43/14; the diagnostics builders and request-ADU-aware validator add strict FC07/FC08/FC0B/FC0C master support.
...
| Write File Record | `0x15` | 27 subrequests; 251 request-data bytes | TCP and RTU |
...
copying or dynamically allocating record storage. See
[`docs/modbus-fc20-read-file-record.md`](docs/modbus-fc20-read-file-record.md)
for the storage API, wire format, master validation, and limits.

FC21 writes one or more sequential ranges in the same application-owned file
arrays. The complete request is validated before any record is modified, the
normal response exactly echoes the request, and RTU broadcast writes are
supported without a response. See
[`docs/modbus-fc21-write-file-record.md`](docs/modbus-fc21-write-file-record.md).

FC22 atomically combines one holding register with an AND mask and an OR mask,
returns an exact request echo for unicast requests, and supports RTU broadcast
writes without a response. See
[`docs/modbus-fc22-mask-write-register.md`](docs/modbus-fc22-mask-write-register.md).
...
└── src/
    ├── modbus.c              Coils, registers, Device ID, and file-map state
...
│   ├── test_modbus_fc22.c    Dedicated FC22 mask/echo/broadcast tests
...
├── modbus-fc22-mask-write-register.md
├── modbus-fc23.md
...
FC0F, FC10, FC11, FC20, FC21, FC22, FC23, and FC43/14 request ADUs and validates one complete response against
...
acknowledgements, FC23 read data, FC43/14 object lists, and Modbus
exception responses.
